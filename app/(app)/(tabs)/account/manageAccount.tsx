...
function manageAccount() {
  const [name, setName] = useState("Nishit Verma");
  const [gender, setGender] = useState("Male");
  const [date, setDate] = useState("08/12/1995");
  const [number, setNumber] = useState("+91 7037244534");
  const [email, setEmail] = useState("mathuyrgopal@gmail.com");
    ...
    async function getUserDetails() {
      ...
      if (res) {
        ...
        if (userDetails) {
          ...
          setNumber((prev) => (userDetails.mobile ? userDetails.mobile : prev));
